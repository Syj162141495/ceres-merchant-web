  ...
  <div class="pending">
    <div class="formSearch">
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item>
          ...
          <el-button type="primary" plain @click="addDiscountFn">新增</el-button>
        </el-form-item>
      </el-form>
    </div>
    <!-- 表格 -->
    <div class="tableBox">
      ...
      >
        <el-table-column label="活动名称" width="220">
          <template slot-scope="scope">{{ scope.row.discountName }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" show-overflow-tooltip>
          <template slot-scope="scope">
            <div class="btnList">
              <el-button v-if="scope.row.state === 0" type="text" @click="edit(scope.row.shopDiscountId)">编辑</el-button>
    ...
    >
      ...
      <div class="dataEffect">
        <h3>实时统计</h3>
        <div class="couponTit">活动名称：{{ dataInfo.discountName }}</div>
...
<script>
import { getDiscountData, deleteDiscount, stopDiscount, getDiscountInfo } from '@/api/marketing'
export default {
  name: 'Discount',
  data() {
    return {
  ...
  methods: {
    ...
    edit(id) {
      this.$router.push({
        name: 'addDiscount',
        query: { shopDiscountId: id }
      })
    },
    // 停止秒杀活动
    stopFn(id) {
      stopDiscount({ shopDiscountId: id }).then(res => {
        if (res.code === '') {
          this.$message({
            message: '停止成功',
            type: 'success'
    ...
    async getAll(formInline) {
      const res = await getDiscountData(formInline)
    ...
    getActiveData(id) {
      ...
      getDiscountInfo(this.activeDataParams).then(res => {
        if (res.code === '') {
