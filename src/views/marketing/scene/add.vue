<template>
  <div class="addCoupon">
    ...
    <div class="addCouponBox">
      <div class="addCouponTit">{{ sceneId ? '修改场景营销' : '新增场景营销' }}</div>
        ...
        <el-form-item class="item-time" label="营销时间" prop="sceneTime">
          ...
          <div v-if="addForm.sceneType === 2">
            <dl class="dl-month">
              ...
              <dd>
                每月<el-input v-model="monthStart" type="number" oninput="value=value.replace(/[^\d]/g,'')" />日 至 每月<el-input v-model="monthEnd" type="number" oninput="value=value.replace(/[^\d]/g,'')" />日
              </dd>
            </dl>
            <dl>
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    ...
                    <el-input-number v-model="scope.row.discount" :precision="1" :min="1" :max="10"></el-input-number> 折
                  </template>
                </el-table-column>
                <el-table-column
                  prop="ids"
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button @click="goToScene">取 消</el-button>
    </span>
    <!-- 新建分组弹框 -->
    <el-dialog
    ...
    addCouponFn(ruleForm) {
      this.$refs[ruleForm].validate(valid => {
        if (valid) {
          ...
          console.log(this.addForm, this.vipList, this.vipList2)
          if (this.sceneId) {
        ...
        if (res.code === '') {
          ...
          for (let i = 0; i < row.ids.length; i++) {
            this.tableData.forEach(item => {
              if (item.shopCouponId === row.ids[i]) {
