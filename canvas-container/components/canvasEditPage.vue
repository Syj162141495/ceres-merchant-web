<template>
   <div class="layout hom-layout">
     <draggable
       ...
       @change="pageChange"
     >
     <div class="list-group-item" v-for="(item,index) in componentsData" :key="index" :class="{'on':activeComponent == index}" @click="selectComponent(item,index)">
       <component v-show="!item.isEmpty" :isNoData.sync='item.isEmpty' :is="componentMap[terminal-1].get(item.type)" :componentContent="item.componentContent" :terminal="terminal" :typeId="typeId" :shopId="shopId"></component>
    ...
    methods: {
      ...
      pageChange (e) {
        if (e.added) {
          ...
          // var _=this
          // let typeArr = ['banner','custom','productList','groupList','spikeList','discountList','priceList','vip','coupon','newProduct','shop'] // 需要判断是否为空的组件
          // typeArr.forEach(function(value){
          //   if(_.componentsData[e.added.newIndex].type == value){
          //     _.componentsData[e.added.newIndex].isNoData = true
          //   }
          // })
        }
        if (e.moved) {
