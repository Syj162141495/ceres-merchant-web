    ...
    <div class="toolBox">
      <div class="selectBox">
        ...
        <div class="layoutList">
          <span class="item iconfont" :class="{active: activeComponent.componentContent.layoutType === item.type}" @click="selectLayout(item, index)" v-for="(item, index) of layoutList" :key="item.id" v-html="item.Icon"></span>
        </div>
      </div>
      ...
      <div class="itemBox">
        <div v-if="activeComponent.componentContent.layoutType !== 'average'">
          ...
          >
            <div class="item" :class="{active: activeComponent.componentContent.imgBoxActive === index-1}" @click="changeLayout(index-1)" v-for="index of activeComponent.componentContent.elementNum" :key="index">
              <!--<span>宽度375像素</span>-->
              <img class="img" :src="activeComponent.componentContent.imgData[index-1].src" v-show="activeComponent.componentContent.imgData[index-1].src">
            </div>
          </div>
        </div>
        <div v-else>
          <dl class="densityLiist">
            ...
            <dd>
              <el-select v-model="activeComponent.componentContent.density" placeholder="" @change="densityChange">
                ...
                <el-option label="10*10" value="10"></el-option>
              </el-select>
            </dd>
          </dl>
          <div class="textTit">布局</div>
          ...
          <div class="averageBoxWarp">
            <div class="averageBox" @mouseleave="averageBoxLeave">
              <ul v-for="i of parseInt(activeComponent.componentContent.density)" :key="'y'+i" :class="'col'+activeComponent.componentContent.density">
                <li v-for="j of parseInt(activeComponent.componentContent.density)" :key="'x'+j" :class="{'on':activeComponent.componentContent.averageBoxData[i-1][j-1].hover}" @click="averageBoxClick(i-1,j-1)" @mouseover="averageBoxMouseover(i-1,j-1)"></li>
              </ul>
            </div>
            <div class="selectedCube">
              <ul>
                <li v-for="(item,index) of activeComponent.componentContent.imgData" :class="{active: activeComponent.componentContent.imgBoxActive === index}" :key="index" :style="{'width':getItemValue(item.width) + '%','height':getItemValue(item.height) + '%','left':getItemValue(item.left) + '%','top':getItemValue(item.top) + '%'}">
                 <div class="box" @click="changeLayout(index)">
                   <span>
                    {{parseInt(getItemValue(item.width) * 12)}}x{{parseInt(getItemValue(item.height) * 12)}}
                  </span>
                   <img class="img" :src="activeComponent.componentContent.imgData[index].src" v-if="activeComponent.componentContent.imgData[index].src">
                 </div>
                  <a class="btn-close" @click="delsSlectedCube(item,index)"><i class="icon iconfont icon-close"></i></a>
                </li>
              </ul>
            </div>
          </div>
